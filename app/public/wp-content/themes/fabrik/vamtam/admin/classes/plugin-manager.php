...
class VamtamPluginManager {
		...
		<div class="vamtam-ts">
			...
			<?php if ( $valid_key ) : ?>
				<div class="vamtam-plugin-manager vamtam-has-required-plugins">
					<h2><?php esc_html_e( 'Recommended Plugins', 'fabrik' ); ?></h2>
					...
					if ( count( $plugins['all'] ) ) {
						?>
						<p><?php echo sprintf( esc_html( 'You can install exclusive and recommended plugins for %s here, and add or remove them later on WordPress plugins page.', 'fabrik'), $theme_name ); ?></p>
								...
								<thead>
									<tr>
										<td id="cb" class="manage-column column-cb check-column" width="10%">
											...
											for="cb-select-all"><?php esc_html_e( 'Select All', 'fabrik' ); ?></label>
											<input
											id="cb-select-all" type="checkbox">
										</td>
										<th class="manage-column column-thumbnail"></th>
										<th scope="col" id="name"
										class="manage-column column-name" width="15%"><?php esc_html_e( 'Name', 'fabrik' ); ?></th>
										<th scope="col" id="description"
										class="manage-column column-description" width="50%"><?php esc_html_e( 'Description', 'fabrik' ); ?></th>
										<th scope="col" id="status"
										class="manage-column column-status" width="17%"><?php esc_html_e( 'Status', 'fabrik' ); ?></th>
										<th scope="col" id="version"
										class="manage-column column-version" width="8%"><?php esc_html_e( 'Version', 'fabrik' ); ?></th>
									</tr>
								</thead>
									...
									?>
										<tr class="vamtam-plugin" data-slug="<?php echo esc_attr( $slug ); ?>" data-category="<?php echo esc_attr( $plugin['category'] ); ?>">
											...
											<?php
												...
												if ( strpos( $slug, 'fabrik' ) !== false ) {
													echo '<div class="vamtam-label">' . esc_html__('Exclusive', 'fabrik') . '</div>';
												}
											?>
											</td>
												...
												<span>
													<?php
														if ( isset( $plugins['install'][ $slug ] ) ) {
															echo esc_html__( 'Not Installed', 'fabrik' );
														} elseif ( isset( $plugins['activate'][ $slug ] ) ) {
															echo esc_html__( 'Not Activated', 'fabrik' );
														}
													?>
												</span>
											</td>
						...
						<a id="install-plugins-btn" href="#"
							...
							data-callback="install_plugins"><?php esc_html_e( 'Install Plugins', 'fabrik' ); ?></a>
						</div>

					<?php
					} else { ?>
						<div class="vamtam-plugins-success">
							<?php echo esc_html__( 'All plugins are already installed and up to date.', 'fabrik'  ); ?>
						</div>
					<?php
					} ?>
				</div>
	...
	public function ajax_plugins() {
		if ( ! check_ajax_referer( 'vamtam_setup_nonce', 'wpnonce' ) || empty( $_POST['slug'] ) ) {
			wp_send_json_error( array( 'error' => 1, 'message' => esc_html__( 'No Slug Found', 'fabrik' ) ) );
		}
		$json = array();
		...
		foreach ( $plugins['activate'] as $slug => $plugin ) {
			if ( $_POST['slug'] == $slug ) {
				$json = array(
					...
					'action2'       => - 1,
					'message'       => esc_html__( 'Activating...', 'fabrik' ),
				);
				break;
			}
		}
		foreach ( $plugins['install'] as $slug => $plugin ) {
			if ( $_POST['slug'] == $slug ) {
				$json = array(
					...
					'action2'       => - 1,
					'message'       => esc_html__( 'Installing...', 'fabrik' ),
				);
				break;
			}
		}
		...
		if ( $json ) {
			...
			wp_send_json( array( 'done' => 1, 'message' => esc_html__( 'Activated', 'fabrik' ) ) );
		}
		exit;
	...
	public static function filter_tabs() {
		...
		<div id="vamtam-plugins-filters">
			<ul>
				<li>
					<a data-filter="required" class="vamtam-filter-btn">
						<?php esc_html_e( 'Required', 'fabrik' ); ?>
					</a>
				</li>
				<li>
					<a data-filter="recommended" class="vamtam-filter-btn">
						<?php esc_html_e( 'Recommended', 'fabrik' ); ?>
					</a>
				</li>
			</ul>
			<hr/>
			<p id="vamtam-required-plugins-notice">
				<strong><?php esc_html_e( 'Why required plugins?', 'fabrik' ); ?></strong>
				<br />
				<?php esc_html_e( 'A plugin offers additional functionality and features beyond a typical WordPress installation. You do not need additional licensing in order to use the plugins below with your theme.', 'fabrik' ); ?>
			</p>
			<p id="vamtam-recommended-plugins-notice">
				...
				<?php esc_html_e( 'Please note that the theme doesn\'t depend on the plugins in this list to function properly. Nor does the demo content importer. Install them only if you are going to use them, otherwise, they may impact the performance of the site or put an unnecessary burden on your hosting.', 'fabrik' ); ?>
			</p>
		</div>
		<?php
	}
