...
class VamtamTemplates {
	...
	public static function pagination_list( $query = null, $format = '', $base = '' ) {
		...
		if ( $total_pages > 1 ) {
			...
			$output .= '<span class="pages screen-reader-text">' . sprintf( esc_html__( 'Page %1$d of %2$d', 'fabrik' ), (int) $current_page, (int) $total_pages ) . '</span>';
			...
			$output .= paginate_links( array( // xss ok
				...
				'total'     => $total_pages,
				'prev_text' => '<span class="screen-reader-text">' . esc_html__( 'Prev', 'fabrik' ) . '</span>',
	...
	public static function page_header( $placed = false, $title = null ) {
		...
		if ( is_null( $title ) ) {
			...
			if ( empty( $title ) ) {
				$title = esc_html__( 'Untitled', 'fabrik' );
			}
		}
	...
	public static function the_author_posts_link_with_icon() {
		...
		$link = sprintf(
			...
			esc_attr( sprintf( __( 'Posts by %s', 'fabrik' ), get_the_author() ) ),
